...
def log_param_stats(params):
    """Recursively log norm, NaN and Inf counts for each leaf array in params."""
    def _check(node, prefix=""):
        if isinstance(node, dict):
            ...
            for idx, v in enumerate(node):
                _check(v, f"{prefix}/{idx}")
        else:
            try:
                arr = jnp.array(node)
            except Exception:
                logging.info("Skipped %s of type %s", prefix, type(node))
                return
            norm = float(jnp.linalg.norm(arr))
            nan = int(jnp.isnan(arr).sum())
            inf = int(jnp.isinf(arr).sum())
            logging.info("Param %s: norm=%.6f, n_nan=%d, n_inf=%d", prefix, norm, nan, inf)
    _check(params)


def _mask_nan_updates(old_params, new_params):
    """Masks NaNs in parameter updates: replaces NaN updates with zeros."""
    def mask(o, n):
        delta = n - o
        delta = jnp.nan_to_num(delta, nan=0.0)
        return o + delta
    return jax.tree_util.tree_map(mask, old_params, new_params)
...
def train(
	...
	def minibatch_step(
		...
		# zero out any NaN updates
		new_params = _mask_nan_updates(old_params, new_params)
	...
	if process_id == 0 and num_evals > 1 and run_evals:
		...
		# log parameter stats after initial eval
		log_param_stats(_unpmap((
			training_state.normalizer_params,
			training_state.params.policy,
			training_state.params.value,
		)))
	...
	for it in range(num_evals_after_init):
		...
		if num_evals > 0:
			...
			logging.info(metrics)
			# log parameter stats after each eval
